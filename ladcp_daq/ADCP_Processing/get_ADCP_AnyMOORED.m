function [ADdata] = get_ADCP_AnyMOORED(beg_time, end_time, SNparamsFile, index_file, data_path, flag_ok)
%    [ADdata] = get_ADCP_Any(beg_time, end_time, index_file, data_path);
%    returns data from any matlab ADCP data
%
% input:
...
%    InP - infile for specific ADCP deployment (set_ADCPsn_params_cruiesname.m).
...
%  Mickett modified slightly for mooring data 7/2/09
...
if nargin<3 | isempty(index_file)
	disp(['get_ADCP_AnyMOORED: Inputs beg_time,end_time,index_file are required.']);
    return
end


if beg_time<datenum(InP.Cruise.StartTime) | end_time>datenum(InP.Cruise.EndTime) | beg_time>=end_time;
            disp(['get_ADCP_AnyMOORED: The data record start time of ' datestr(beg_time)]);
              disp(['or end time ' datestr(end_time) ' are outside of data bounds or are equal.']);
            return
        end

% if beg_time<0 | end_time>540 | beg_time>=end_time
%     
%     
%    disp(['get_ADCP_AnyMOORED: beg_time,end_time are out of range/order.']);
%    return
% end
...
if (end_time-beg_time)*24>48 & flag_ok~=1
   disp('get_ADCP_AnyMOORED: Too much data requested, exceeds 48 hours');
   return
end
...
if(exist(index_file) == 0)
   disp('get_ADCP_AnyMOORED:  Index file does not exist.');
...
for i=1:length(Set_params)
   yb = Set_params(i).start_dnum;
   ye = Set_params(i).end_dnum;
...
if ~iSet
   disp(['get_ADCP_AnyMOORED: Index does not cover specified time range'])
   return
end
...
yd_jump_aft = [];
yd_jump_by = [];
if isfield(Set_params(iSet), 'dnum_jump_after')
    yd_jump_aft = [Set_params(iSet).dnum_jump_after,  inf]; % last one to adjust to end
    yd_jump_by = Set_params(iSet).dnum_jump_adjust;
end
yd_off = [];
if isfield(Set_params(iSet), 'ping_seconds_offset')
    yd_off = Set_params(iSet).ping_seconds_offset / 86400; 
elseif isfield(Set_params(iSet), 'dnum_seconds_offset')
    yd_off = Set_params(iSet).dnum_seconds_offset / 86400; 
end
if isempty(yd_off)
    yd_off = 0;
end
...
yb = Index(iSet).dnum_beg; ye = Index(iSet).dnum_end;
...
if isempty(ifil)
   disp(['get_ADCP_AnyMOORED: Index has no files for specified time range'])
   return
end
...
for ifil = 1:length(fyb)
    ...
    elseif ~isempty(SN)
        ...
        if strcmp(SN,'SWIMS_Vels') 
            ...
            XX.SWIMS_Vels.yday = XX.SWIMS_Vels.dnum_beg;
...
% AD.sysconfig=XX.Vel.sysconfig;
% AD.params=XX.Vel.params;
% AD.mfiles=XX.mfiles;
% 
% clear XX
...
for i=1:length(yd_jump_by)
    ix = find( AD.dnum>=yd_jump_aft(i) & AD.yday<yd_jump_aft(i+1) );
    AD.dnum(ix) = AD.dnum(ix) + yd_jump_by(i);
end
AD.dnum = AD.dnum + yd_off;
...
index = find(AD.dnum >= beg_time & AD.dnum <= end_time);
if ~isempty(index)
    for i=1:length(vars)
        if ~isempty(vars{i})
            ok = eval(['size(AD.' vars{i} ', 2) == length(AD.dnum)']);
