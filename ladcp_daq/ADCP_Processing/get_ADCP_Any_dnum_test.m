function [ADdata] = get_ADCP_Any_dnum_test(beg_time, end_time, infile, data_path, flag_ok)
...
% input:
%    beg_time   - lower datenum limit for which data are to be retrieved.
%    end_time   - upper datenum limit for which data are to be retrieved.
%    infile - the mat infile that has the particulars for the deployment
%    ('SN----_params_CRUISENAME.mat');
...
% check for existence of infile, if it exists load it
% otherwise return with an error and explanation.
if(exist(infile) == 0)
   disp('get_ADCP_Any_dnum:  Input parameter file does not exist.');
