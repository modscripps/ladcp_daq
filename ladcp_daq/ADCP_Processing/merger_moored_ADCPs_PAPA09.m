...
%from PAPA08 mooring.
load('/Users/johnmickett/Cruises_Research/PAPA/PAPA09/ADCP/Data/SN10923/data_mat/SN10923_papa09_AllData.mat');
Vel1=Vel;
load('/Users/johnmickett/Cruises_Research/PAPA/PAPA09/ADCP/Data/SN11181/data_mat/SN11181_papa09_AllData.mat');
Vel2=Vel;
clear Vel
...
%                v: [100x17562 double]
%                w: [100x17562 double]
%            ecAve: [100x17562 double]
%            dtnum: [1x17562 double]
%             yday: [1x17562 double]
%                z: [100x1 double]
%             temp: [1x17562 double]
%     xducer_depth: [1x17562 double]
%         botdepth: 4235
%              lat: 50.114666666666665
%              lon: -1.449710000000000e+02
%       MagDecUsed: 17.433333333333334
%             info: [1x1 struct]
%setting time and depth grid.

%gridding to 
Iee=(nanmedian(diff(Vel1.dtnum))); %half hourly on 1/2 hours
Igg=(nanmedian(diff(Vel2.dtnum))); %half-hourly on 1/2 hours
%gridding to finer grid so as not to lose data

Dtvec=Vel2.dtnum; %using longer record..

%preserving vertical resolution, so patching both z-grids.
chop=4; %bad data at the bottom of the top ADCP.


Ise=find(Vel2.z>Vel1.z(end-chop));
...
Vmrg=[V1I(1:end-chop,:);V2new];
Umrg=[U1I(1:end-chop,:);U2new];
Wmrg=[W1I(1:end-chop,:);W2new];
Ecmrg=[Ec1I(1:end-chop,:);Ec2new];


figure
pcolorjbm(Dtvec,Znew,Umrg);caxis([-0.5 0.5]);colormap(redblue);
