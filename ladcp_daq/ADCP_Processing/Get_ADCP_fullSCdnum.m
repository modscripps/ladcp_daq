function Vel = Get_ADCP_fullSCdnum(RDIname, filetype, savetype, ens_ind_rng);
...
%		starting with RDIname (more later);  -1 for one file with merged
%		ensembles.
...
if filetype < 1 % either: 0 = solo profile, or <0 = one file, multi-profs
   raw2mat_SCv3(RDIname,TMPfil, [], {'.mat'}, [], ens_ind_rng);
    ...
    error('Get_ADCP_fullSCdnum.m only works for filetype<0')
end
...
Vel.dnum = datenum(year+2000,month,day,hour,minute,second+hundreths/100);
...
if savetype>0 
    ...
    if ~isempty( find(~isnan([pg1(:); pg2(:); pg3(:); pg4(:)])))
